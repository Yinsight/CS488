...
public class SenderStop {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		while (fis.read(data) != -1) {
			...
			DatagramPacket receivedACK = new DatagramPacket(receiveACK, receiveACK.length);
			datagramSocket.setSoTimeout(30000);
			datagramSocket.receive(receivedACK);
			System.out.println("Ack received.");
			
			timedOut = false;
		} catch (SocketTimeoutException exception) {			
			//>>Stop and Select aka Sliding Window = 1 
			//>>Send packet, receive ack. Catch timeout (packet loss), resend packet. 
			//Therefore, no need for array
		}
